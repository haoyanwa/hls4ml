...
int main(int argc, char **argv) {
    ...
    if (fin.is_open() && fpr.is_open()) {
        ...
        for (; std::getline(fin, iline) && std::getline(fpr, pline); iteration++) {
            ...
            float vals[N_INPUT_1_1]; 
            for (int j = 0 ; j < N_INPUT_1_1 ; j++) {
                vals[j] = in[j]; 
            }
            nnet::convert_data<float, DenseInputPipe, N_INPUT_1_1>(q, vals);

            q.single_task(Myproject{});

            // hls-fpga-machine-learning convert output
            float outputs[N_LAYER_2];
            nnet::convert_data_back<Layer4OutPipe, float, N_LAYER_2>(q, outputs);
